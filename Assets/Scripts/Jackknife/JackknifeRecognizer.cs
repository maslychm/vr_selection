...
namespace Jackknife
{
    public class Jackknife
    {
        ...
        private Dictionary<int, int> templatesPerID;
        ...
        public Jackknife(JackknifeBlades blades)
        {
            ...
            templatesPerID = new Dictionary<int, int>();
        }

        public string Summarize(Dictionary<int, string> idToName)
        {
            var tCnt = templates.Count;
            var sortedGids = templatesPerID.Keys.OrderBy(x => x).ToList();

            var ret = $"Loaded {tCnt} templates of {templatesPerID.Count} gestures\n";
            foreach (var gid in sortedGids)
            {
                var gname = idToName[gid];
                ret += $"{gname} (gid={gid}) : {templatesPerID[gid]}\n";
            }
        ...
        public void AddTemplate(Sample sample)
        {
            ...
            if (templatesPerID.ContainsKey(t.GestureId))
            {
                templatesPerID[t.GestureId] += 1;
            }
            else
            {
                templatesPerID.Add(t.GestureId, 1);
            }
        }
        ...
        */
        ...
        public double TestClassificationGetAccuracy()
        {
            // Sanity check - each trajectory should be classified as itself
            double correct = 0.0, total = 0.0;
            for (int t = 0; t < templates.Count; t++)
            {
                if (Classify(templates[t].Sample) == templates[t].GestureId)
                {
                    correct += 1;
                }
                total += 1;
